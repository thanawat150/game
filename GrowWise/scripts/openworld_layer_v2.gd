...
const INTERACT_DISTANCE: float = 92.0
const RESOURCE_DISTANCE: float = 64.0
...
var collected_days: Dictionary = {}
var npc_last_talk: Dictionary = {}
...
func _ready() -> void:
	...
	super._ready()
	var result: Dictionary = GrowWiseOpenWorldSystem.self_test(openworld_data)
	if bool(result.get("ok", false)):
		print("GROWWISE_OPENWORLD_OK")
	else:
		push_error("Open-world self-test failed: %s" % JSON.stringify(result))

func tx(key_name: String) -> String:
	var labels: Dictionary = {
		"ui.openworld":{"th":"โลกกว้าง","en":"Open World"},
		"ui.world_map":{"th":"แผนที่โลก","en":"World Map"},
		"ui.travel_energy":{"th":"พลังเดินทาง","en":"Travel Energy"}
	}
...
func new_game() -> void:
	...
	discovered_regions = {"greenfield":true}
	discovered_points = {"farm_gate":true}
	collected_days = {}
	npc_last_talk = {}
	world_stamina = 100.0
	world_mount = "none"
...
func build_buttons() -> void:
	...
	buttons = kept
	buttons.append(button("openworld_toggle", Rect2(946.0, 492.0, 58.0, 52.0), "eco", "ui.openworld"))
...
func move_player(delta: float) -> void:
	...
	if world_mount == "bicycle": speed_value *= 1.42
	if world_mount == "utility_cart": speed_value *= 1.78
	var previous_position: Vector2 = world_position
	...
	world_travel_distance += previous_position.distance_to(world_position)
	...
	if mode == "game" and event is InputEventKey and event.pressed and not event.echo:
		...
		if openworld_active:
			if key_event.keycode == KEY_TAB:
				...
				return
			if key_event.keycode == KEY_E and overlay.is_empty():
				interact_open_world()
				return
			if key_event.keycode == KEY_T and overlay.is_empty():
				toggle_world_mount()
				return
			if key_event.keycode == KEY_ESCAPE and overlay == "world_map":
				overlay = ""
				return
...
func world_color(value: String, fallback: Color) -> Color:
...
func draw_world_regions() -> void:
	for value: Variant in GrowWiseOpenWorldSystem.array_value(openworld_data, "regions"):
		...
		var screen_rect: Rect2 = screen_rect_from_world(region_rect)
		if not screen_rect.intersects(Rect2(-200.0, -200.0, 1680.0, 1120.0)):
			continue
		var fill_color: Color = world_color(String(region.get("color", "7fbf66")), GREEN)
		draw_rect(screen_rect, fill_color)
		draw_rect(screen_rect.grow(-12.0), fill_color.lightened(0.06), false, 3.0)
		draw_text(localized_world_value(region, "name"), screen_rect.position + Vector2(28.0, 42.0), 18, Color(1.0, 1.0, 1.0, 0.55), 360.0)

func route_screen_points(route: Array) -> PackedVector2Array:
	var result: PackedVector2Array = PackedVector2Array()
	for point_value: Variant in route:
		var pair: Array = point_value as Array
		if pair.size() >= 2:
			result.append(world_to_screen(Vector2(float(pair[0]), float(pair[1]))))
	return result

func draw_world_roads() -> void:
	for route_value: Variant in GrowWiseOpenWorldSystem.array_value(openworld_data, "roads"):
		var points: PackedVector2Array = route_screen_points(route_value as Array)
		if points.size() >= 2:
...
func draw_world_resources() -> void:
	for value: Variant in GrowWiseOpenWorldSystem.array_value(openworld_data, "resources"):
		...
		if int(collected_days.get(resource_id, -999)) == day:
			continue
		var resource_position: Vector2 = Vector2(float(resource.get("x", 0.0)), float(resource.get("y", 0.0)))
		var screen_position: Vector2 = world_to_screen(resource_position)
		if not Rect2(-60.0, -60.0, 1400.0, 840.0).has_point(screen_position):
			continue
		var resource_type: String = String(resource.get("type", "fiber"))
		var fill_color: Color = Color("6f9a50")
		match resource_type:
			"wood": fill_color = Color("6d4d34")
			"stone": fill_color = Color("858783")
			"scrap": fill_color = Color("7c8588")
			"mineral": fill_color = Color("b78bd1")
			"herb": fill_color = Color("57a55a")
			"fiber": fill_color = Color("c0b06b")
			"clay": fill_color = Color("b66f4d")
			"seed": fill_color = Color("e0b847")
		draw_circle(screen_position + Vector2(3.0, 8.0), 17.0, Color(0.1, 0.15, 0.1, 0.28))
		draw_circle(screen_position, 14.0, fill_color)
		draw_circle(screen_position - Vector2(4.0, 4.0), 4.0, fill_color.lightened(0.35))

func building_index(point_type: String) -> int:
	match point_type:
		"farm": return 0
		"machinery": return 1
		"animals": return 2
...
func draw_world_points() -> void:
	for value: Variant in GrowWiseOpenWorldSystem.array_value(openworld_data, "points"):
		...
		var screen_position: Vector2 = world_to_screen(point_position)
		if not Rect2(-150.0, -150.0, 1580.0, 1020.0).has_point(screen_position):
			continue
		var texture_index: int = building_index(String(point.get("type", "farm")))
		if texture_index >= 0 and texture_index < building_textures.size():
			draw_texture_rect(building_textures[texture_index], Rect2(screen_position - Vector2(48.0, 80.0), Vector2(96.0, 96.0)), false)
		var point_id: String = String(point.get("id", ""))
		if bool(discovered_points.get(point_id, false)) or world_position.distance_to(point_position) < 190.0:
			draw_text(localized_world_value(point, "name"), screen_position + Vector2(-90.0, 32.0), 13, CREAM, 180.0, HORIZONTAL_ALIGNMENT_CENTER)
		if bool(point.get("fast_travel", false)) and bool(discovered_points.get(point_id, false)):
			draw_circle(screen_position + Vector2(34.0, -45.0), 7.0, GOLD)
...
func draw_world_npcs() -> void:
	...
	for index: int in range(npc_values.size()):
		...
		var npc_world_position: Vector2 = GrowWiseOpenWorldSystem.npc_position(npc, minutes)
		var screen_position: Vector2 = world_to_screen(npc_world_position)
		if not Rect2(-80.0, -80.0, 1440.0, 880.0).has_point(screen_position):
			continue
		var texture_index: int = int(npc.get("texture", index))
		if texture_index >= 0 and texture_index < npc_textures.size():
			draw_texture_rect(npc_textures[texture_index], Rect2(screen_position - Vector2(24.0, 48.0), Vector2(48.0, 48.0)), false)
		draw_text(localized_world_value(npc, "name"), screen_position + Vector2(-60.0, 18.0), 11, CREAM, 120.0, HORIZONTAL_ALIGNMENT_CENTER)

func draw_world_wildlife() -> void:
	if creature_textures.is_empty():
		return
	var locations: Array[Vector2] = [Vector2(-280.0, -630.0), Vector2(420.0, -810.0), Vector2(-1130.0, 420.0), Vector2(-380.0, 820.0), Vector2(980.0, -610.0), Vector2(360.0, 260.0)]
	for index: int in range(locations.size()):
		...
		var screen_position: Vector2 = world_to_screen(locations[index] + offset_value)
		if Rect2(-50.0, -50.0, 1380.0, 820.0).has_point(screen_position):
			var texture_index: int = posmod(index + day, creature_textures.size())
			draw_texture_rect(creature_textures[texture_index], Rect2(screen_position - Vector2(18.0, 26.0), Vector2(36.0, 36.0)), false)

func draw_world_player() -> void:
	var shadow_width: float = 22.0 if world_mount == "none" else 32.0
	draw_ellipse(WORLD_CENTER + Vector2(0.0, 19.0), shadow_width, 9.0, Color(0.08, 0.12, 0.08, 0.35))
...
func draw_open_world_hud() -> void:
	...
	draw_text(localized_world_value(region, "name"), Vector2(34.0, 47.0), 20, CREAM, 350.0)
	...
	draw_text("%s %.0f/100 • %s" % [tx("ui.travel_energy"), world_stamina, mount_name], Vector2(34.0, 98.0), 13, GOLD, 350.0)
	...
	if not world_event.is_empty():
		...
		draw_text("เหตุการณ์วันนี้: %s" % localized_world_value(world_event, "name"), Vector2(440.0, 48.0), 14, INK, 420.0, HORIZONTAL_ALIGNMENT_CENTER)
...
func draw_world_minimap(rect_value: Rect2) -> void:
	...
	for value: Variant in GrowWiseOpenWorldSystem.array_value(openworld_data, "regions"):
		...
		if not bool(discovered_regions.get(region_id, false)):
			continue
		var region_rect: Rect2 = GrowWiseOpenWorldSystem.rect_from(GrowWiseOpenWorldSystem.dictionary_value(region, "rect"))
		...
		draw_rect(Rect2(top_left, bottom_right - top_left), world_color(String(region.get("color", "7fbf66")), GREEN))
...
func draw_world_map_overlay() -> void:
	...
	for value: Variant in GrowWiseOpenWorldSystem.array_value(openworld_data, "regions"):
		...
		if bool(discovered_regions.get(region_id, false)):
			fill_color = world_color(String(region.get("color", "7fbf66")), GREEN)
		...
		if bool(discovered_regions.get(region_id, false)):
			draw_text(localized_world_value(region, "name"), top_left + Vector2(6.0, 17.0), 11, Color.WHITE, maxf(80.0, bottom_right.x - top_left.x - 12.0))
	for value: Variant in GrowWiseOpenWorldSystem.array_value(openworld_data, "points"):
		var point: Dictionary = value as Dictionary
		var point_id: String = String(point.get("id", ""))
		if not bool(point.get("fast_travel", false)) or not bool(discovered_points.get(point_id, false)):
			continue
		var point_map: Vector2 = GrowWiseOpenWorldSystem.map_position(GrowWiseOpenWorldSystem.point_position(point), bounds, WORLD_MAP_RECT)
		draw_circle(point_map, 9.0, GOLD)
		draw_circle(point_map, 12.0, INK, false, 2.0)
		draw_text(localized_world_value(point, "name"), point_map + Vector2(-65.0, 25.0), 10, INK, 130.0, HORIZONTAL_ALIGNMENT_CENTER)
...
func handle_world_map_click(position_value: Vector2) -> void:
	...
	for value: Variant in GrowWiseOpenWorldSystem.array_value(openworld_data, "points"):
		...
		if not bool(point.get("fast_travel", false)) or not bool(discovered_points.get(point_id, false)):
			continue
		var point_map: Vector2 = GrowWiseOpenWorldSystem.map_position(GrowWiseOpenWorldSystem.point_position(point), bounds, WORLD_MAP_RECT)
		if position_value.distance_to(point_map) <= 18.0:
			var target: Vector2 = GrowWiseOpenWorldSystem.point_position(point)
			var distance_value: float = world_position.distance_to(target)
			world_position = target + Vector2(0.0, 72.0)
			world_camera = world_position
			...
			while minutes >= 1440.0:
				...
				advance_day()
			overlay = ""
			notify("เดินทางถึง %s" % localized_world_value(point, "name"), "success")
			return
...
func update_world_discovery() -> void:
	...
	if not region.is_empty():
		...
		if not bool(discovered_regions.get(region_id, false)):
			...
			world_stats["discover"] = discovered_regions.size()
			add_farm_xp(35, "ค้นพบพื้นที่ใหม่", WORLD_CENTER)
			notify("ค้นพบ: %s" % localized_world_value(region, "name"), "success")
	for value: Variant in GrowWiseOpenWorldSystem.array_value(openworld_data, "points"):
		var point: Dictionary = value as Dictionary
		var point_id: String = String(point.get("id", ""))
		if bool(discovered_points.get(point_id, false)):
			continue
		if world_position.distance_to(GrowWiseOpenWorldSystem.point_position(point)) <= 150.0:
			discovered_points[point_id] = true
			knowledge += 1
			add_feedback("พบสถานที่: %s" % localized_world_value(point, "name"), WORLD_CENTER + Vector2(0.0, -80.0), GOLD)

func update_world_prompt() -> void:
	if not overlay.is_empty():
		world_prompt = ""
		return
	var point: Dictionary = GrowWiseOpenWorldSystem.nearest_point(openworld_data, world_position, INTERACT_DISTANCE)
	if not point.is_empty():
		world_prompt = "[E] %s • %s" % [localized_world_value(point, "name"), String(point.get("description", ""))]
		return
	var resource: Dictionary = GrowWiseOpenWorldSystem.nearest_resource(openworld_data, world_position, RESOURCE_DISTANCE, collected_days, day)
	if not resource.is_empty():
		world_prompt = "[E] เก็บทรัพยากรธรรมชาติ"
		return
	var npc: Dictionary = nearest_world_npc(82.0)
	if not npc.is_empty():
		world_prompt = "[E] พูดคุยกับ %s" % localized_world_value(npc, "name")
		return
...
func collect_world_resource(resource: Dictionary) -> void:
	...
	collected_days[resource_id] = day
...
func talk_world_npc(npc: Dictionary) -> void:
	...
	var already_talked: bool = int(npc_last_talk.get(npc_id, -1)) == day
	var dialogue: String = "ยินดีต้อนรับสู่ชุมชน GrowWise"
	match npc_id:
		"teacher": dialogue = "ลองสังเกตดิน น้ำ และอากาศก่อนเลือกพืชนะ"
		"merchant": dialogue = "ตลาดต้องการผลผลิตคุณภาพสูงและของแปรรูป"
		"researcher": dialogue = "เก็บตัวอย่างหลายพื้นที่ แล้วเปรียบเทียบผลแล็บ"
		"fisher": dialogue = "สภาพอากาศและฤดูกาลมีผลต่อปลาที่พบ"
		"ranger": dialogue = "เก็บทรัพยากรได้ แต่อย่าลืมรักษาความหลากหลาย"
		"engineer": dialogue = "น้ำมากระบายลงบ่อ น้ำขาดค่อยส่งกลับเข้าแปลง"
	if not already_talked:
		npc_last_talk[npc_id] = day
		knowledge += 1
		if npc_id == "fisher":
			inventory["bait"] = int(inventory.get("bait", 0)) + 1
			dialogue += " • รับเหยื่อตกปลา 1 ชิ้น"
		add_farm_xp(5, "พูดคุยกับชุมชน", WORLD_CENTER)
	notify("%s: %s" % [localized_world_value(npc, "name"), dialogue], "success")
...
func update_world_quest() -> void:
	...
	notify("สำเร็จ: %s • +%d เงิน" % [localized_world_value(quest, "title"), reward_money], "success")
	world_quest_index += 1
...
func current_world_quest_text() -> String:
	...
	return "ภารกิจ: %s • %d/%d" % [localized_world_value(quest, "title"), progress_value, target_value]

func localized_world_value(source: Dictionary, prefix: String) -> String:
...
func advance_day() -> void:
	...
	world_event = GrowWiseOpenWorldSystem.active_event(openworld_data, day)
	if not world_event.is_empty():
		notify("เหตุการณ์โลก: %s" % localized_world_value(world_event, "name"), "success")
...
func save_game(slot_number: int, automatic: bool) -> bool:
	...
	payload["world_discovered_regions"] = discovered_regions
	payload["world_discovered_points"] = discovered_points
	payload["world_collected_days"] = collected_days
	payload["world_npc_last_talk"] = npc_last_talk
...
func load_game(slot_number: int) -> bool:
	...
	discovered_regions = dictionary_value(payload, "world_discovered_regions", {"greenfield":true})
	discovered_points = dictionary_value(payload, "world_discovered_points", {"farm_gate":true})
	collected_days = dictionary_value(payload, "world_collected_days", {})
	npc_last_talk = dictionary_value(payload, "world_npc_last_talk", {})
