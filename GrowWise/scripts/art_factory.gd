...
static func build_atlas() -> Image:
	var image: Image = Image.create_empty(1024, 512, false, Image.FORMAT_RGBA8)
...
static func _draw_icon(image: Image, ox: int, oy: int, kind: String) -> void:
	var ink: Color = Color("29302a")
	var wood: Color = Color("b97a4d")
...
static func _draw_player(image: Image, ox: int, oy: int, index: int) -> void:
	var bob: int = index % 2
	var skin: Color = Color("e8ad76")
	var hat: Color = Color("e9b84d")
	var denim: Color = Color("365f8c")
	var ink: Color = Color("29302a")
