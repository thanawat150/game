...
func refresh_workforce_candidates(force: bool) -> void:
	if not force and workforce_candidate_day > 0 and day - workforce_candidate_day < 3:
		return
...
func run_workforce_tick() -> void:
	...
	for offset: int in range(hired_workers.size()):
		...
		if bool(worker.get("resting", false)) or float(worker.get("fatigue", 0.0)) >= 95.0:
			continue
		var action_limit: int = GrowWiseWorkforce.daily_action_limit(workforce_data, worker)
		if int(worker.get("actions_today", 0)) >= action_limit:
			continue
		var result: Dictionary = perform_worker_action(worker)
		if bool(result.get("ok", false)):
			worker["actions_today"] = int(worker.get("actions_today", 0)) + 1
			var trait: Dictionary = GrowWiseWorkforce.trait_definition(workforce_data, String(worker.get("trait_id", "")))
			var fatigue_gain: float = 14.0 * (1.0 + float(trait.get("fatigue", 0.0)))
			...
			worker = GrowWiseWorkforce.apply_experience(workforce_data, worker, 8 + int(result.get("value", 0)) / 10)
...
func perform_worker_action(worker: Dictionary) -> Dictionary:
	match String(worker.get("role", "farmhand")):
...
func worker_irrigation_action() -> Dictionary:
	var wettest_key: String = ""
	var wettest_value: float = 88.0
	var driest_key: String = ""
	var driest_value: float = 46.0
	for key_value: Variant in tiles.keys():
		...
		if moisture > wettest_value:
			wettest_value = moisture
			wettest_key = key_string
		if moisture < driest_value:
			driest_value = moisture
			driest_key = key_string
	if not wettest_key.is_empty():
		var wet_tile: Dictionary = dictionary_value(tiles, wettest_key)
		wet_tile["moisture"] = maxf(35.0, float_value(wet_tile, "moisture") - 18.0)
		tiles[wettest_key] = wet_tile
	...
	if not driest_key.is_empty():
		var parts: PackedStringArray = driest_key.split(",")
		if parts.size() == 2:
...
func worker_animal_action() -> Dictionary:
	...
	for item_id: String in dictionary_value(animal_state, "pending_products"):
		pending_total += int(dictionary_value(animal_state, "pending_products")[item_id])
...
func worker_driver_action() -> Dictionary:
	if not dictionary_value(logistics_state, "active_trip").is_empty():
		return {"ok": false}
	var before_trip: Dictionary = dictionary_value(logistics_state, "active_trip")
	start_city_delivery()
	var after_trip: Dictionary = dictionary_value(logistics_state, "active_trip")
	if before_trip.is_empty() and not after_trip.is_empty():
		return {"ok": true, "text": "ออกส่งสินค้าเข้าเมือง", "value": int(after_trip.get("cargo_value", 50))}
	return {"ok": false}
...
func workforce_role_name(role_id: String) -> String:
	...
	return String(definition.get("name_th" if language == "th" else "name_en", role_id))

func workforce_trait_name(worker: Dictionary) -> String:
	return String(worker.get("trait_name_th" if language == "th" else "trait_name_en", ""))
...
func overlay_click(position: Vector2) -> void:
	if overlay == "workforce":
		...
		if Rect2(245, 515, 170, 46).has_point(position):
			...
			return
		if Rect2(425, 515, 190, 46).has_point(position):
			if money >= 10:
				money -= 10
				expenses += 10
				workforce_candidate_day = 0
				refresh_workforce_candidates(true)
			return
		if Rect2(650, 515, 115, 46).has_point(position):
			change_selected_worker_role()
			return
		if Rect2(775, 515, 105, 46).has_point(position):
			toggle_selected_worker_rest()
			return
		if Rect2(890, 515, 140, 46).has_point(position):
			dismiss_selected_worker()
			return
		if Rect2(650, 570, 380, 42).has_point(position):
...
func draw_workforce_overlay() -> void:
	...
	for index: int in range(mini(6, hired_workers.size())):
		...
		draw_text("%s • %s%s" % [String(worker.get("name", "-")), workforce_role_name(String(worker.get("role", ""))), " • พัก" if bool(worker.get("resting", false)) else ""], rect_value.position + Vector2(8, 20), 14, GREEN)
...
func draw_world() -> void:
	...
	var role_positions: Dictionary = {
		"farmhand": Vector2(2.0, 5.7),
		"irrigator": Vector2(4.2, 1.6),
		"animal_keeper": Vector2(8.2, 6.0),
		"processor": Vector2(8.3, 4.5),
		"driver": Vector2(8.8, 0.8),
		"field_technician": Vector2(7.7, 2.4)
	}
	for index: int in range(mini(5, hired_workers.size())):
		var worker: Dictionary = hired_workers[index] as Dictionary
		var position: Vector2 = role_positions.get(String(worker.get("role", "farmhand")), Vector2(3.0 + index, 5.5)) as Vector2
		draw_texture(npc_textures[index % npc_textures.size()], iso(position) - Vector2(32, 58))
		draw_text(String(worker.get("name", "")), iso(position) + Vector2(-36, -62), 10, CREAM, 72.0, HORIZONTAL_ALIGNMENT_CENTER)
