...
public enum EntityAction { Idle, SearchFood, Eat, Hunt, Flee, Gather, Build, Farm, Migrate, Defend, Trade, Pray, Heal }
...
public enum GodPowerType { CreateForest, Rain, Fertility, Knowledge, Peace, Lightning, Fire, Plague, Meteor, Blessing, Curse }
public enum WorldAge { Dawn, Growth, Kingdoms, Conflict, Ash, Frost, Wonders, Silence }

public sealed record TraitDefinition(string Id, float HealthBonus = 0, float SpeedBonus = 0, float IntelligenceBonus = 0, float FertilityBonus = 0, bool Heritable = true);
...
public sealed class SimEntity
{
    ...
    public float Loyalty { get; set; } = 50;
...
public sealed class SettlementState
{
    ...
    public SettlementStage Stage { get; set; } = SettlementStage.Camp;
    public int X { get; set; }
    public int Y { get; set; }
    public ulong? KingdomId { get; set; }
    ...
    public float Security { get; set; } = 20;
...
public sealed class KingdomState
{
    ...
    public float Stability { get; set; } = 60;
    public float Economy { get; set; } = 10;
...
public sealed class GrandSimulation
{
    ...
    private readonly Dictionary<string, TraitDefinition> _traits = new(StringComparer.Ordinal)
    {
        ["trait.strong"] = new("trait.strong", HealthBonus: 20),
        ["trait.swift"] = new("trait.swift", SpeedBonus: 2),
        ["trait.clever"] = new("trait.clever", IntelligenceBonus: 5),
        ["trait.fertile"] = new("trait.fertile", FertilityBonus: 0.02f),
        ["trait.blessed"] = new("trait.blessed", HealthBonus: 10, IntelligenceBonus: 2, Heritable: false),
        ["trait.cursed"] = new("trait.cursed", HealthBonus: -15, Heritable: false),
    };
    ...
    public SimEntity SpawnEntity(SpeciesKind species, int x, int y, string? name = null)
    {
        ...
        if (species is not SpeciesKind.Fish && terrain is TerrainType.DeepOcean or TerrainType.ShallowWater)
            throw new InvalidOperationException("Land entities cannot spawn in water.");
        if (species == SpeciesKind.Fish && terrain is not (TerrainType.DeepOcean or TerrainType.ShallowWater))
    ...
    {
        if (entity.SettlementId is ulong settlementId && State.Settlements.TryGetValue(settlementId, out SettlementState? settlement) && settlement.Food >= 2)
        {
            settlement.Food -= 2;
            entity.Hunger = MathF.Max(0, entity.Hunger - 65);
            entity.Health = MathF.Min(MaxHealth(entity), entity.Health + 2);
            return true;
        }
    ...
    {
        if (entity.SettlementId is not ulong settlementId || !State.Settlements.TryGetValue(settlementId, out SettlementState? settlement)) return;
        TerrainType terrain = _world.GetTerrain(entity.X, entity.Y);
    ...
    public SettlementState FoundSettlement(IEnumerable<ulong> settlerIds, string name)
    {
        ...
        AddEvent("settlement.founded", "Settlement founded", $"{name} was founded by {settlers.Length} settlers.", x, y, 3, settlers.Select(e => e.Id).ToArray());
        return settlement;
    }
    ...
    public KingdomState FoundKingdom(ulong capitalSettlementId, string name, GovernmentType government)
    {
        ...
        AddEvent("kingdom.founded", "Kingdom founded", $"{name} emerged with {capital.Name} as its capital.", capital.X, capital.Y, 4, kingdom.RulerId is ulong ruler ? new[] { ruler } : Array.Empty<ulong>());
        return kingdom;
    }

    public void SetRelation(ulong firstKingdomId, ulong secondKingdomId, int value)
    {
        if (firstKingdomId == secondKingdomId) throw new ArgumentException("A kingdom cannot have diplomacy with itself.");
        value = Math.Clamp(value, -100, 100);
        State.Kingdoms[firstKingdomId].Relations[secondKingdomId] = value;
        State.Kingdoms[secondKingdomId].Relations[firstKingdomId] = value;
    }

    public RelationState GetRelationState(ulong firstKingdomId, ulong secondKingdomId)
    {
        int value = State.Kingdoms[firstKingdomId].Relations.GetValueOrDefault(secondKingdomId);
    ...
    public void ApplyPower(GodPowerType power, int x, int y, int radius = 2)
    {
        ...
        foreach (SimEntity entity in State.Entities.Values.Where(e => e.IsAlive && DistanceSquared(e.X, e.Y, x, y) <= radius * radius))
        {
            switch (power)
            {
                case GodPowerType.Blessing: entity.Health = MathF.Min(MaxHealth(entity), entity.Health + 30); if (!entity.Traits.Contains("trait.blessed")) entity.Traits.Add("trait.blessed"); break;
    ...
    private void UpdateDiseases()
    {
        ...
        {
            foreach (ulong entityId in disease.InfectedDays.Keys.OrderBy(id => id).ToArray())
            {
                if (!State.Entities.TryGetValue(entityId, out SimEntity? entity) || !entity.IsAlive) { disease.InfectedDays.Remove(entityId); continue; }
                int day = ++disease.InfectedDays[entityId];
                ...
                if (day >= disease.DurationDays) disease.InfectedDays.Remove(entityId);
            }
    ...
    private void AdvanceMonth()
    {
        ...
        foreach (KingdomState kingdom in State.Kingdoms.Values)
        {
            ...
            if (kingdom.Stability < 20) AddEvent("kingdom.rebellion_risk", "Rebellion risk", $"{kingdom.Name} is close to rebellion.", State.Settlements[kingdom.CapitalId].X, State.Settlements[kingdom.CapitalId].Y, 4);
    ...
    private void TryReproduce(SimEntity[] citizens)
    {
        int freeHousing = citizens.Select(c => c.SettlementId).Distinct().Sum(id => id is ulong sid ? State.Settlements[sid].Housing : 0) - citizens.Length;
        if (freeHousing <= 0 || citizens.Length < 2) return;
        SimEntity[] adults = citizens.Where(c => c.AgeDays >= 18 * 360 && c.Health > 40).OrderBy(c => c.Id).ToArray();
        if (adults.Length < 2 || _random.NextFloat() > adults.Average(c => c.Fertility)) return;
        SimEntity first = adults[0]; SimEntity second = adults[1];
        SimEntity child = SpawnEntity(SpeciesKind.Settler, first.X, first.Y);
        child.SettlementId = first.SettlementId; child.KingdomId = first.KingdomId; child.Parents.Add(first.Id); child.Parents.Add(second.Id);
        foreach (string trait in first.Traits.Intersect(second.Traits).Where(id => _traits.GetValueOrDefault(id)?.Heritable == true)) if (_random.NextFloat() < 0.5f) child.Traits.Add(trait);
        AddEvent("entity.born", "Birth", $"{child.Name} was born.", child.X, child.Y, 1, child.Id, first.Id, second.Id);
    }
    ...
    private void TryResearch(KingdomState kingdom, int population)
    {
        ...
        if (_random.NextFloat() < chance) { kingdom.Technologies.Add(next); AddEvent("technology.discovered", "Technology discovered", $"{kingdom.Name} discovered {next}.", State.Settlements[kingdom.CapitalId].X, State.Settlements[kingdom.CapitalId].Y, 3); }
    }

    private static void UpgradeSettlement(SettlementState settlement, int population)
    {
        settlement.Stage = population switch { >= 100 => SettlementStage.City, >= 50 => SettlementStage.Town, >= 15 => SettlementStage.Village, _ => settlement.Stage };
    }
    ...
    private bool IsSuitableSettlementTile(int x, int y)
    {
        ...
        TerrainType terrain = _world.GetTerrain(x, y);
        if (terrain is TerrainType.DeepOcean or TerrainType.ShallowWater or TerrainType.Mountain) return false;
        for (int dy = -3; dy <= 3; dy++) for (int dx = -3; dx <= 3; dx++) if (_world.IsInside(x + dx, y + dy) && _world.GetTerrain(x + dx, y + dy) == TerrainType.ShallowWater) return true;
        return terrain is TerrainType.Grassland or TerrainType.Forest;
    }

    private float MaxHealth(SimEntity entity) => 100 + entity.Traits.Sum(id => _traits.GetValueOrDefault(id)?.HealthBonus ?? 0);
...
{
    private static readonly HashSet<string> Supported = new(StringComparer.OrdinalIgnoreCase) { "species", "traits", "buildings", "technologies", "biomes", "disasters", "powers", "cultures", "religions", "items", "events", "sprites", "sounds" };
    public static IReadOnlyList<string> Validate(ModManifest manifest, IEnumerable<string> installedModIds)
    {
        ...
        if (!Version.TryParse(manifest.Version, out _)) errors.Add("Mod version must be a valid semantic version.");
