...
public sealed partial class LivingWorldMainController : Node2D
{
    private void BuildRightPanel()
    {
        ...
        foreach (Control control in new Control[]
                 {
                     _cityPriority, _cityBorder, _cityTax, _cityBirth, _cityFoodReserve,
                     _cityPopulationLimit, _cityAutoBuild, _cityQuarantine, _cityEvacuate,
                 })
        {
            if (control is OptionButton option)
                option.ItemSelected += _ => ApplySelectedCityPolicy();
            else if (control is Godot.Range range)
                range.ValueChanged += _ => ApplySelectedCityPolicy();
            else if (control is BaseButton button)
                button.Toggled += _ => ApplySelectedCityPolicy();
        }
        ...
        foreach (Control control in new Control[]
                 {
                     _kingdomBorder, _kingdomTax, _kingdomBirth, _kingdomMilitary,
                     _kingdomPopulationLimit, _kingdomPreferPeace,
                 })
        {
            if (control is OptionButton option)
                option.ItemSelected += _ => ApplySelectedKingdomPolicy();
            else if (control is Godot.Range range)
                range.ValueChanged += _ => ApplySelectedKingdomPolicy();
            else if (control is BaseButton button)
                button.Toggled += _ => ApplySelectedKingdomPolicy();
        }
        ...
        _scenarioProgress = new ProgressBar
        {
            MinValue = 0,
            MaxValue = 100,
            Value = 0,
